...
def write_instance_to_example_files(instances, tokenizer, max_seq_length, max_predictions_per_seq, output_files):
  ...
  for (inst_index, instance) in enumerate(instances):
    input_ids = tokenizer.convert_tokens_to_ids(instance.tokens) #向量化在此步骤(mask 处理后的)
    ...
    next_sentence_label = 1 if instance.is_random_next else 0 #应对 sentence pair型任务

    features = collections.OrderedDict()
    features["input_ids"] = create_int_feature(input_ids) # mask处理后的的vector
    features["input_mask"] = create_int_feature(input_mask) # 表示向量有效位的向量, pad位取零
    features["segment_ids"] = create_int_feature(segment_ids) # 标识句子
    features["masked_lm_positions"] = create_int_feature(masked_lm_positions)
    features["masked_lm_ids"] = create_int_feature(masked_lm_ids)
    features["masked_lm_weights"] = create_float_feature(masked_lm_weights)
    features["next_sentence_labels"] = create_int_feature([next_sentence_label])# label形式进入model
...
def create_training_instances(input_files, tokenizer, max_seq_length, dupe_factor, short_seq_prob, masked_lm_prob,
                              max_predictions_per_seq, rng, nsp=False):
  ...
  for input_file in input_files:
    with tf.gfile.GFile(input_file, "r") as reader:
      while True:
        ...
        tokens = tokenizer.tokenize(line) # list
        if tokens:
...
def create_instances_from_document(all_documents, document_index, max_seq_length, short_seq_prob, masked_lm_prob,
  ...
  if not nsp:
    ...
    max_num_tokens += 1
    len_calcu = min(len(text), max_num_tokens)
    tokens = ["[CLS]"]+text[0:len_calcu]+["[PAD]"]*(max_num_tokens-len_calcu)+["[SEP]"]
    ...
    return [TrainingInstance(tokens=tokens, segment_ids=segment_ids, is_random_next=is_random_next,
  ...
  while i < len(document):
    ...
    if i == len(document) - 1 or current_length >= target_seq_length:
      if current_chunk:
        ...
        # (first) sentence.
        # what if len(current_chunk[0]) > target_seq_length
        a_end = 1
        ...
        instance = TrainingInstance(tokens=tokens, segment_ids=segment_ids, is_random_next=is_random_next,
                                    masked_lm_positions=masked_lm_positions, masked_lm_labels=masked_lm_labels)
...
def create_masked_lm_predictions(tokens, masked_lm_prob, max_predictions_per_seq, vocab_words, rng):
  ...
  for (i, token) in enumerate(tokens):
    ...
    if (FLAGS.do_whole_word_mask and len(cand_indexes) >= 1 and token.startswith("##")):
      ...
      cand_indexes.append([i]) # 关于词的边界问题, count(piece)>=count(word)

  rng.shuffle(cand_indexes) # 达到随机抽取的目的
  ...
  for index_set in cand_indexes:
    ...
    for index in index_set:
      ...
      if rng.random() < 0.8:
        ...
        if rng.random() < 0.5:    #0.1 正确填充
          masked_token = tokens[index]
        # 10% of the time, replace with random word
        else:
          masked_token = vocab_words[rng.randint(0, len(vocab_words) - 1)] # 0.1错误填充
